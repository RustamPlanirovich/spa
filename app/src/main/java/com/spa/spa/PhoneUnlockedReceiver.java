...
public class PhoneUnlockedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        if (intent.getAction().equals(Intent.ACTION_USER_PRESENT)) {
            //Запускаем таймер на отработку каждые 300 миллисекунд на обнаружение действия
            //разблокировки
            ...
            handler.postDelayed(new Runnable() {
                public void run() {
                    ...
                    //Присваиваем переменной brig1 значение после прочтения файла настройки
                    int brig1 = sp.getInt(APP_PREFERENCES_NAME, 0);
                    //Устанавливаем считаенное значение для яркости экрана после разблокировки
                    //экрана
